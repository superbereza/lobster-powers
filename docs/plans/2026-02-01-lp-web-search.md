# lp-web-search Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Web search with Brave and Perplexity providers.

**Architecture:** Provider abstraction with API key detection. Brave for traditional results, Perplexity for AI-synthesized answers.
...
**Files:**
- Create: `src/lobster_powers/tools/web_search.py`
...
```toml
lp-web-search = "lobster_powers.tools.web_search:main"
```

Note: Uses same `httpx` dependency from `web` group.
...
lp-web-search "python async tutorial"
lp-web-search "latest news" --freshness pd --count 3
```

**Step 4: Commit**

```bash
git add src/lobster_powers/tools/web_search.py pyproject.toml
git commit -m "feat(web-search): add lp-web-search with Brave/Perplexity"
```
...
```bash
git add skills/web-search.md
git commit -m "docs(web-search): add skill file"
```
...
Total: 2 tasks, ~180 lines of code.
